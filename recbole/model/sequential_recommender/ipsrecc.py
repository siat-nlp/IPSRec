...
from recbole.model.layers import TransformerEncoder, BiTransformerEncoder
...
class IPSRecC(SequentialRecommender):
    r"""
    ...
    def __init__(self, config, dataset):
        super(IPSRecC, self).__init__(config, dataset)
        ...
        self.trm_encoder = BiTransformerEncoder(
    ...
    def forward(self, item_seq, seller_seq, item_seq_len):
        ...
        input_emb_item = self.LayerNorm1(input_emb_item)
        input_emb_item = self.dropout1(input_emb_item)

        input_emb_seller = self.LayerNorm2(input_emb_seller)
        input_emb_seller = self.dropout2(input_emb_seller)

        extended_attention_mask_item = self.get_attention_mask_item(item_seq)
        extended_attention_mask_seller = self.get_attention_mask_seller(seller_seq)

        trm_output1, trm_output2 = self.trm_encoder(input_emb_item, extended_attention_mask_item,
                                                    input_emb_seller, extended_attention_mask_seller,
                                                    output_all_encoded_layers=True)
        output1 = trm_output1[-1]
        output1 = self.gather_indexes(output1, (item_seq_len - 1))

        output2 = trm_output2[-1]
