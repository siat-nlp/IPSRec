...
class Dataset(object):
    ...
    def _get_field_from_config(self):
        ...
        self.pid_field = self.config['SELLER_ID_FIELD']
        ...
        self.logger.debug(set_color('pid_field', 'blue') + f': {self.pid_field}')
    ...
    def provider_num(self):
        ...
        Returns:
            int: Number of different tokens of ``self.iid_field``.
        """
        self._check_field('pid_field')
        return self.num(self.pid_field)
    ...
    @property
    def avg_actions_of_providers(self):
        """Get the average number of items' interaction records.
        ...
        """
        if isinstance(self.inter_feat, pd.DataFrame):
            return np.mean(self.inter_feat.groupby(self.pid_field).size())
        else:
            return np.mean(list(Counter(self.inter_feat[self.pid_field].numpy()).values()))
    ...
    def __str__(self):
        ...
        if self.pid_field:
            info.extend([
                set_color('The number of providers', 'blue') + f': {self.provider_num}',
                set_color('Average actions of providers', 'blue') + f': {self.avg_actions_of_providers}'
            ])
